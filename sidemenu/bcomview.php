...
<div class="marginl" >
		...
		<div class="admincontent fleft">
			<div>
					...
					echo "<div class='fleft insert sphover'><i class='fa fa-remove'></i>
							".$_GET['deleted']."
					</div>";
					}
...
if ($result->num_rows > 0) {
     ...
     while($row = $result->fetch_assoc()) {	$del=$row["sid"]; $edit=$row["sid"];$det=$row["sid"];
		...
		<div class='pic' id='pic".$i."' ><img src='". $row["image_path"] . $row["image_name"] ."' alt='". $row["image_name"] ."' width=150  height=auto ></div>
									...
									<li>
							<a href='bcomedited.php?edit=$edit'><i title='Edit the Record of ".$row["student_name"]."' class='fa fa-edit fa-lg fa-fw'></i></a>
							<a href='bcomdelete.php?del=$del'><i title='Delete the Record of ".$row["student_name"]."' class='fa fa-remove fa-lg fa-fw'></i></a>
							<a href='bcomdetail.php?detail=$det'><i title='View Full Detail of ".$row["student_name"]."' class='fa fa-info-circle fa-lg fa-fw'></i></a>
									</li>
								</ul>
